...
As I don't expect to be using resource-heavy services such as multiple desktop VM:s, 4K-encoding or multi-user streaming I've settled for [intel Celeron J4105](https://www.intel.com/content/www/us/en/products/sku/128989/). Despite being launched at the end of 2017 it has a set of very useful features for my use case:
...
Ideally, the server will be running 24/7, 365 days a year. As such, PSU efficiency is important too keep upkeep-cost down.

A power supply is at its most efficient at 50% of maximum rated load, this means a 500 W PSU rated at 480 W max load will be at its most efficient when it provides 240 W of power.

__How efficient is it at 50% load?__ That is determined by your PSU's [80+ Rating](https://en.wikipedia.org/wiki/80_Plus). A 80+ Bronze will be 85-88% efficient at 50% load while a 80+ Titanium will be 94-96% efficient.

After some "back of the napkin calculations" I've estimated my system to draw 5 W at idle and 25 W under heavy load. As such my ideal PSU would be a power-brick style 60-100 W PSU. I've had no luck finding a reliable model and have instead opted for a 300 W [SYSTEM POWER B9](https://www.bequiet.com/en/powersupply/1285) from _be quiet!_. It offers a better low-load efficiency than its 450-500 W counterparts while still offering some head-room if I were to upgrade my system.
